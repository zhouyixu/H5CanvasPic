...
var draw_graph = function(graphType,obj){	
	...
	$(canvas_bak).bind('dblclick',addMouseDown);
}
...
var clearContext = function(type){
	...
	var addMouseDown = function(evt){
        console.log("add");
         var mousePos = getMousePos(canvas_bak, evt);
            var word = prompt("请输入文字注释");
       		if(word != null){
       			    var x = canvas_bak.width / 2;
        		    var y = canvas_bak.height / 2;
        		    context.font = "15pt Microsoft JhengHei";            //设置文本大小 + 字体
       			    context.fillStyle = "tomato";                        //设置文本颜色
        			context.fillText(word, mousePos.x, mousePos.y); 
       		}
      }

        //获取鼠标指针坐标
        function getMousePos (context_bak, evt) {
            var rect = canvas_bak.getBoundingClientRect();
            return {
